% Computes the baseball traveling, based on a more realistic drag model.
% Precondition: n/a.
...
function res = drag_force(V)
    ...
    c_d = drag_coeff(V);    % drag coefficient; dimensionless
...
function res = drag_coeff(V)
    if V <= 26          % velocity in m/s
        res = 0.5;
    elseif V <= 36
        res = 0.4;
    else
